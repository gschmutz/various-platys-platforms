...
curl -XPOST -H "Content-Type:application/json" -d"{\"schema\":$SCHEMA}" http://localhost:8081/subjects/$TOPIC-value/versions
```
...
```bash
export TOPIC=topicName
http http://localhost:8081/subjects/$TOPIC-value/versions/latest \
